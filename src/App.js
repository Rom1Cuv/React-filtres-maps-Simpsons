...
function App() {
  ...
  const [showSimpsonsQuoteOnly, setShowSimpsonsQuoteOnly] =
    React.useState(true);

  function changeStatusClick() {
    setShowSimpsonsQuoteOnly(!showSimpsonsQuoteOnly);
  }

  return (
    <div>
      <button onClick={changeStatusClick}>Simpson family quote only</button>
      {quoteList
        .filter(
          (member) =>
            !showSimpsonsQuoteOnly || member.character.includes('Simpson')
        )
        .map((member, index) => (
          <QuoteCard key={index} {...member} />
        ))}
    </div>
  );
}
